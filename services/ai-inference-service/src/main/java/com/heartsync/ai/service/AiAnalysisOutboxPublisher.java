    ...
    public void publishPending() {
        ...
        for (AiAnalysisOutboxMessage message : messages) {
            try {
                ...
                meterRegistry.counter("heartsync.ai_analysis_requested.published", "source", "ai-inference-service").increment();
            } catch (Exception e) {
                message.setStatus(AiAnalysisOutboxMessage.Status.FAILED);
                message.setAttempts(message.getAttempts() + 1);
                message.setLastError(e.getMessage());
                meterRegistry.counter("heartsync.ai_analysis_requested.publish_failed", "source", "ai-inference-service").increment();
